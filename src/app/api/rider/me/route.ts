...
export async function GET() {
  ...
  try {
    ...
    const { riderId, phoneDigits, meta } = await resolveRiderIdFromUser(supabase, user, decoded);
    console.info("[rider/me] resolve result", {
      riderId,
      phone: phoneDigits ? maskPhone(phoneDigits) : null,
      userId: user?.id,
      hasServiceKey: Boolean(auth.serviceSupabase),
    });

    // 메타데이터 보완: rider_id/phone이 비어있으면 채워둠(서비스 키가 있을 때만)
    if (auth.serviceSupabase && riderId && meta?.rider_id !== riderId) {
      try {
        await auth.serviceSupabase.auth.admin.updateUserById(user.id, {
          user_metadata: {
            ...meta,
            rider_id: riderId,
            phone: phoneDigits || meta?.phone || null,
          },
        });
      } catch (updateErr) {
        console.warn("[rider/me] failed to sync user metadata:", updateErr);
      }
    }
    ...
    if (riderErr) {
      ...
      return NextResponse.json(
        { error: "라이더 정보를 불러오지 못했습니다.", code: "rider_query_failed" },
        { status: 500 }
      );
    }
    if (!riderRow) { 
      console.log('라이더로우',riderRow,riderId)
      return NextResponse.json(
        { error: "라이더 정보를 찾을 수 없습니다!.", code: "rider_not_found" },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { error: "라이더 정보를 불러오는 중 오류가 발생했습니다.", code: "rider_me_unknown" },
      { status: 500 }
    );
  }
}
